...
export default function LeafThree({
  ...
  const stemClipRectRef = useRef<SVGRectElement>(null)
  const stemFillRef = useRef<SVGPathElement>(null)
  ...
  useScrollProgressEffect(
    (progress) => {
      ...
      fillCircleRef.current?.setAttribute('r', String(155 * fillProgress))

      if (stemClipRectRef.current) {
        stemClipRectRef.current.setAttribute('y', String(2159 - stemClipHeight))
        stemClipRectRef.current.setAttribute('height', String(stemClipHeight))
      }

      if (stemFillRef.current) {
        stemFillRef.current.style.opacity = String(stemOpacity)
      }
    ...
    >
      <defs>
        <clipPath id="leaf-three-stem-clip">
          <rect ref={stemClipRectRef} x="0" y="2159" width="1569" height="0" />
        </clipPath>
        <clipPath id="leaf-three-fill-reveal">
          <circle ref={fillCircleRef} cx="681" cy="89" r="0" />
        </clipPath>
      </defs>
      <path
